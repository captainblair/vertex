        ...
        if (user) {
          try {
            const order = await orderService.createOrder(
              user.id,
              total,
              "Nairobi Delivery Node", // Mock address for now
              cart.map(item => ({
            ...
            await orderService.logMpesaTransaction(
              order.id,
            ...
            console.error("Critical: Order logging failed, but payment succeeded", dbError);
          }
        }
    ...
    return (
      <div className="max-w-2xl mx-auto py-56 px-6 text-center space-y-12 min-h-screen">
        ...
        <div className="space-y-6">
          <h2 className="text-3xl md:text-5xl font-black tracking-tighter uppercase leading-none text-dark">Order Confirmed!</h2>
          <p className="text-zinc-500 text-lg font-medium leading-relaxed max-w-md mx-auto">Payment successful. Your order is confirmed and will be prepared for delivery.</p>
        </div>
        <Button onClick={() => navigate('/')} variant="primary" className="px-12 py-5 bg-dark text-white">Continue Shopping</Button>
      </div>
    );
  }
        ...
        <div className="space-y-16 order-2 lg:order-1">
          <div className="flex flex-col gap-12">
            <button onClick={() => navigate('/')} className="w-fit flex items-center gap-3 text-[11px] font-black uppercase tracking-[0.5em] text-zinc-400 hover:text-dark transition-all group">
              <ArrowLeft size={18} className="group-hover:-translate-x-2 transition-transform" /> Back to Cart
            </button>

            <div className="space-y-8">
              <h3 className="text-[12px] font-black uppercase tracking-[0.5em] text-zinc-500 border-b border-zinc-100 pb-6">Your Order</h3>
                  ...
                  <div key={item.product.id} className="flex justify-between items-center gap-8 group">
                    <div className="flex gap-8 items-center">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <p className="text-[10px] text-zinc-400 font-mono font-bold uppercase tracking-[0.2em]">Order ID: {item.product.id.substr(0, 6)}</p>
                      </div>
                    </div>
            ...
            <div className="space-y-6 pt-12 border-t border-zinc-100">
              ...
              <div className="flex justify-between text-2xl md:text-5xl font-black tracking-tighter pt-12 border-t border-dark text-dark">
                <span>Total</span>
        ...
        <div className="space-y-16 order-1 lg:order-2">
          <div className="bg-zinc-100/50 border border-zinc-100 rounded-[3rem] lg:p-16 shadow-plinth space-y-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tighter">Payment</h1>
            ...
            <div className="space-y-10">
              ...
              <div className="p-8 bg-zinc-50 border border-zinc-100 rounded-[2.5rem] flex items-center justify-between group cursor-default shadow-inner">
                <div className="flex items-center gap-8">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-zinc-400 font-medium tracking-wide">Instant Payment Available</p>
                  </div>
                </div>
              ...
              {/* Secure Input Area */}
              <div className="space-y-6">
                <label className="text-[11px] font-black text-zinc-400 uppercase tracking-[0.6em] ml-4">Phone Number</label>
                ...
                <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-[0.3em] text-center lg:text-left ml-4">You will receive a payment prompt on your phone.</p>
              </div>
              ...
              {checkoutStatus !== 'IDLE' && checkoutStatus !== 'SUCCESS' && (
                <div className="p-8 border border-zinc-100 bg-zinc-50/50 rounded-[2.5rem] space-y-6">
                  ...
                  <div className="flex items-center justify-center gap-3 text-brand">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em]">Processing payment...</span>
                  </div>
                </div>
              )}
              ...
              {checkoutStatus === 'IDLE' && (
                <Button variant="primary" className="w-full py-10 rounded-[3rem] text-[12px] font-black tracking-[0.4em] shadow-2xl hover:bg-zinc-800 transition-all active:scale-[0.99] flex items-center justify-center gap-4" onClick={handleAcquire}>
                  <Lock size={20} strokeWidth={2.5} /> Pay with M-PESA
                </Button>
              )}
              ...
              {checkoutStatus === 'FAILED' && (
                <div className="p-10 bg-red-50/50 border border-red-100 rounded-[3rem] text-center space-y-8">
                  ...
                  <p className="text-[11px] text-red-600 font-black uppercase tracking-[0.4em]">Payment Failed</p>
                  <Button variant="outline" onClick={() => setCheckoutStatus('IDLE')} className="w-full py-6 rounded-full border-red-100 text-red-600 hover:bg-red-50">Try Again</Button>
                </div>
              )}
            </div>
          </div>
